...
class ChapterController {
    ...
    /// Fetches all chapters from the database, sorted by their order index.
    ...
    /// Fetches all sentences for a specific chapter using the UUID array in your Chapter model.
    func fetchSentences(for chapter: Chapter) -> [Sentence] {
        do {
            let allSentences = try context.fetch(FetchDescriptor<Sentence>())
            
            // This logic correctly filters sentences based on the UUIDs in chapter.sentences
    ...
    /// Fetches all unique words for a given chapter (for the Vocabulary screen).
    ...
    /// Fetches the words for a single sentence in their correct order.
